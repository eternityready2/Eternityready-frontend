...
function placeAds() {
  fetch(`${API_BASE_URL}/api/graphql`, {
    ...
    .then(async result => {
      if (!result.data || !result.data.ads) {
        console.error('Error: unexpected ads response', result);
        return;
      }
      ...
      for (let i = 0; i < shuffledAds.length; i++) {
        ...
        if (ad.status === "now" || ad.status == "active") {
          ...
          if (session) {
            ...
            console.log('User consuming Ad: ', user);

            if (user && user.privilege !== "normal") {
              continue;
            }
          }
          ...
          let adWidth = ad.width.match(/(\d+(\.\d+)?)/);
          adWidth = adWidth ? parseFloat(adWidth[0]) : window.innerWidth;
          ...
          element.style.width = (adWidth > window.innerWidth ? window.innerWidth : adWidth) + 'px';
          ...
          if (ad.mode == "imageLink") {
            ...
            image.src = new URL(ad.image.url, API_BASE_URL).toString();
            ...
            image.addEventListener('click', () => {
              const adWindow = window.open(ad.link, '_blank');
