  ...
  let localData = { channels: [], movies: [], music: [], podcasts: [], radio: [] };
  let normalizedData = { channels: [], movies: [], music: [], podcasts: [], radio: [] };
  ...
  async function loadAllDataSources() {
    const promises = [
      ...
      fetch(`${ETERNITY_BASE_URL}/data/channels.json`), // Local
      fetch(`${ETERNITY_BASE_URL}/data/movies.json`), // Local
      fetch(`${ETERNITY_BASE_URL}/data/music.json`), // Local
      fetch(`${ETERNITY_BASE_URL}/data/radio.json`), // Local Radio
    ];
    ...
    if (results[5].status === "fulfilled") {
      ...
      if (response.ok) {
        ...
        localData.radio = radioData.channels || [];
      } else {
        console.error("Failed fetching radio.json:", response.status);
      }
    } else {
      console.error("Network error fetching radio.json:", results[5].reason);
    }
  ...
  async function initializeSearch(component) {
    console.log(component);
    const input = component.getElementById("search-input-new");
    const dropdown = component.getElementById("search-dropdown");
    if (!input || !dropdown) return;

    const historyList = component.getElementById("history-list"),
      noHistory = component.getElementById("no-history"),
      categoriesList = component.getElementById("categories-list"),
      categoriesSection = component.getElementById("categories-section"),
      historySection = component.getElementById("history-section"),
      mediaSection = component.getElementById("media-section"),
      mediaList = component.getElementById("media-list"),
      trendingList = component.getElementById("trending-list"),
      seeAllLink = component.getElementById("see-all");
      ...
      videos.slice(0, 5).forEach((video) => {
        ...
        switch (video.sourceType) {
          ...
          case "podcasts":
            ...
            break;
          default:
            videoUrl = `/player/?q=${encodeURIComponent(video.title || video.name)}`;
    ...
    const performLiveSearch = async (value) => {
    ...
    document.addEventListener("click", (e) => {
      if (!component.querySelector(".search-container")?.contains(
        e.composedPath()[0])
      ) {
...
async function fetchRecommendation(allMedia) {
  const stored = localStorage.getItem("categoriesConsumed");
  console.log('stored', stored);
  if (!stored) return [];

  const categoriesConsumed = JSON.parse(stored);

  if (Object.keys(categoriesConsumed).length === 0) return [];

  const scored = allMedia
    .map(media => {
      const mediaCategories = media.categories || media.genres || [];
      const score = mediaCategories.reduce((acc, cat) => {
        return acc + (categoriesConsumed[cat.name] || 0);
      }, 0);
      return { media, score };
    })
