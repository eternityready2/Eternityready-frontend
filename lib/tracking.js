...
function getTopTvItems(limit = 10) {
  return getAllSessions()
    .filter(
      session => (session.origin === "channel" || session.origin === "movie")
    )
...
function getRecentlyWatchedTv({ limit = 10, maxAgeMs = 1000 * 60 * 60 * 24 * 7 } = {}) {
  const now = Date.now();

  return getAllSessions()
    .filter(session => session.origin === "channel" || session.origin === "movie")
    .map(session => {
