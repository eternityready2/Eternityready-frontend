...
$page_url = substr($_SERVER["REQUEST_URI"],strrpos($_SERVER["REQUEST_URI"],"/")+1);
$page_uri = explode("?", $page_url);
...
if($page_uri[0] == 'watch.php' && _SEOMOD == 1) {
	$seo_url_redirect = makevideolink($video['uniq_id'], $video['video_title'], $video['video_slug']);
	header("HTTP/1.1 301 Moved Permanently");
	header("Location: ". $seo_url_redirect);
}
