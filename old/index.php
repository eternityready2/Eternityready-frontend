...
/** START [CUSTOM] */
$new_episodes = rfi_get_newest_episode_list(12);
$top_episodes = get_top_episodes_list($config['top_episodes_sort'], $config['top_videos']);
$featured_series = get_series_list(array(), 'RAND()', null, 0, (int) $config['homepage_featured_limit'], null, array(), 'featured', '');
foreach ($featured_series as $k => $v)
{
    $featured_series[$k]['_item_type_'] = 'series';
}
$featured_episodes = get_featured_episodes_list('RAND()', null, 0, (int) $config['homepage_featured_limit'], $total_featured_episodes);

foreach ($featured_episodes as $k => &$v)
{
    $v['_item_type_'] = 'episode';
}


$smarty->clearAllCache(); // Clears all cached output
$smarty->clearCompiledTemplate(); // Clears compiled templates


$smarty->assign('new_episodes', $new_episodes);
$smarty->assign('top_episodes', $top_episodes);
$smarty->assign('featured_series', $featured_series);
$smarty->assign('featured_episodes', $featured_episodes);
/** END [CUSTOM] */
