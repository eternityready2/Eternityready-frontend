    ...
    .then(scheduleData => {
        ...
        function checkMp4Video(url) {
            ...
            if (!/^https?:\/\//i.test(url)) {
                ...
                console.log("Prepending channel. Final URL:", finalUrl);
            }
            return fetch(finalUrl, { method: 'HEAD' }).then(response => {
        ...
        function loadValidatedVideo() {
            let url = getVideoURL(); // getVideoURL() returns the raw URL (e.g., "video.mp4" or a YouTube link)
